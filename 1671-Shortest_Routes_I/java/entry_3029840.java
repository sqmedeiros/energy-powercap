...
class Main {
 public static void main(String args[]) throws Exception {
  ...
  for (int i = 0; i < M; i++) {
   ...
   int to = br.nextInt() - 1;
   Double weight = br.nextInt() * 1.0;
   graph.get(from).add(new Edge(to, weight));
  }

  Double[] results = shortestPaths(graph, 0);
  // BufferedOutputStream writer = new BufferedOutputStream(new
  // FileOutputStream(new File("out.text")));
  BufferedOutputStream writer = new BufferedOutputStream(System.out);
  // writer.write((sccs.size() + "\n").getBytes());
  for (int i = 0; i < N; i++) {
   writer.write(((results[i].longValue()) + " ").getBytes());
  }
  writer.flush();
 }

 private static Double[] shortestPaths(List<List<Edge>> graph, int source) {
  Double[] distances = new Double[graph.size()];
  boolean[] visited = new boolean[graph.size()];
  Arrays.fill(distances, Double.POSITIVE_INFINITY);
  PriorityQueue<Edge> minHeap = new PriorityQueue<>(
    (e1, e2) -> e1.weight > e2.weight ? 1 : e1.weight < e2.weight ? -1 : 0);

  distances[source] = 0.0;
  for (Edge neighbour : graph.get(source)) {
   if (neighbour.to != source) {
    minHeap.add(new Edge(neighbour.to, neighbour.weight));
   }
  }
  ...
  while (!minHeap.isEmpty()) {
   ...
   for (Edge neighbour : graph.get(e.to)) {
    if (distances[neighbour.to] > distances[e.to] + neighbour.weight) {
     minHeap.add(new Edge(neighbour.to, distances[e.to] + neighbour.weight));
    }
   }
  }
