...
public class entry_2363048 {
  ...
  public static class WeightedGraphTraversal {
    private static class DijkstraNode<T> {
      ...
      public DijkstraNode(T value, long cost) {
        this.value = value;
        this.cost = cost;
      }
    }
    ...
    public static <T> Map<T, Long> dijkstra(WeightedGraph<T> graph, T from) {
      ...
      costs.put(from, 0L);
      Queue<DijkstraNode<T>> queue = new PriorityQueue<>(Comparator.comparingLong(a -> a.cost));
      queue.add(new DijkstraNode<>(from, 0L));
      while (!queue.isEmpty()) {
        DijkstraNode<T> curr = queue.poll();
        ...
        for (Edge<T> edge : graph.getEdges(curr.value)) {
          ...
          if (newCost < costs.getOrDefault(edge.to, Long.MAX_VALUE)) {
            ...
            queue.add(new DijkstraNode<>(edge.to, newCost));
          }
        }
      }
      return costs;
