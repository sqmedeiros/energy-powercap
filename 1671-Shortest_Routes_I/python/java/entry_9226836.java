...
public class entry_9226836 {
    ...
    public static void main(String args[])
    {
        ...
        try{
            ...
            long[] dist = ady.Dijsktra(""+0 );

            for(int i=0;i<n;i++)
            {
                writer.print((long)dist[i]+" ");
            }
...
class AdjacencyList implements Graph {
    ...
    @Override
    public long[] BFS_SP(String originVertex) {
        ...
        Queue<Integer> Q = new LinkedList<>();
        long[] parent = new long[vertexCount];
        boolean[] visited = new boolean[vertexCount];
    ...
    */
    ...
    for (int i = 0; i < vertexCount; i++) {
        for (int j = 0; j < vertexCount; j++) {
            if (i == j) {
                distances[i][j] = 0; // The distance from a node to itself is 0
            } else {
                distances[i][j] = INF; // Set the distance initially to infinity
            }
        }
    }

    // Fill the matrix with known direct distances between nodes
    for (int i = 0; i < vertexCount; i++) {
        for (Edge edge : adj.get(i)) {
            distances[i][edge.getDestination()] = Math.min(distances[i][edge.getDestination()], edge.getWeight());
        }
    }
