    ...
    */
    ...
    public static void solve()throws Exception
    {
        ...
        for(int i=0;i<m;i++)
        {
            ...
            for(int i1=0;i1<2;i1++)a[i1]--;
            g.get(a[0]).add(new Edge(a[2],a[1]));
        }
        ...
        while(pq.size()>0)
        {
            long dis1=pq.peek().weight;
            int node=pq.peek().adjnode;
            pq.remove();
            if(dis1>dis[node])continue;
            for(Edge child:g.get(node))
            {
                long edgeWeight=child.weight;
                int adjnode=child.adjnode;
                if(dis1+edgeWeight<dis[adjnode])

                {
                    dis[adjnode]=dis1+edgeWeight;
                    pq.add(new Edge(dis1+edgeWeight,adjnode));
                }
            }
        }
        printArray(dis);
