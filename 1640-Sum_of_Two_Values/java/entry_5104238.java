...
public class entry_5104238
{
    ...
    public static void main (String[] args) throws java.lang.Exception
 {
     ...
     while(T-->0)
     {
      ...
      boolean cc = k%2==0 ? true : false;

      ArrayList<Integer> hidx = new ArrayList<>();

      for(int i=0; i<n; i++) {
       if(cc && arr[i]==k/2) hidx.add(i+1);
       map.put(arr[i], i+1);
      }

      boolean half = hidx.size()>1 ? true : false;

      if(half) {
       put.write(hidx.get(0)+" "+hidx.get(1)+"\n");
       continue;
      }
      ...
      for(Map.Entry<Integer, Integer> ele: map.entrySet()) {
       ...
       int val = ele.getKey();
       if(cc && val==k/2) continue;

       Integer c = map.get(k-val);
       if(c!=null) {
        put.write(ele.getValue()+" "+c+"\n");
        ans = true;
        break;
       }
      }

      if(!ans) put.write("IMPOSSIBLE\n");
