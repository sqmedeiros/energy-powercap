...
//        int t = sc.nextInt();
    ...
    static class Solver {
        ...
        int[] marks;
        boolean[] visited;
        public void solve(InputReader sc, PrintWriter out, int t) {
            int n = sc.nextInt();
            int m = sc.nextInt();
            gp = new ArrayList[n];
            marks = new int[n];
            ...
            for(int i: marks) ansSet.add(i);
//            out.println(Arrays.toString(marks));
            if(ansSet.size() < 2) {
                out.println(0);
                return;
            }
            out.println(ansSet.size()-1);
            sort(marks);
            for(int i = 1; i < n; i++) {
                if(marks[i] != marks[i-1]) {
                    out.println(marks[i]+1 + " " + (marks[i-1]+1));
                }
            }
        }

        private void dfs(int src, int mark) {
            for(int v : gp[src]) {
