...
public class entry_1506063 {
  ...
  while( m --> 0 ){
   ...
   --a;--b;
   arr[a][b] = ( arr[a][b] == -1)? w: Math.min(w, arr[a][b]);
   arr[b][a] = ( arr[b][a] == -1)? w: Math.min(w, arr[b][a]);
  }
  ...
  for(int a = 0; a < n; a++ ){
   for(int b = 0; b < n; b++ ){
    if( a == b ) dis[a][b] = 0;
    else dis[a][b] = (arr[a][b] == -1)? Long.MAX_VALUE: arr[a][b];
   }
  }
  ...
  for(int intermediate = 0; intermediate < n; intermediate++ ){
   for(int b = 0; b < n; b++ ){
    ...
    for(int a = 0; a < n; a ++ ){
     if( dis[a][intermediate] != Long.MAX_VALUE && dis[intermediate][b] != Long.MAX_VALUE){
  ...
  while( q --> 0 ){
   ...
   int b = fastReader.intNext();
   --a;--b;
   println( (dis[a][b] == Long.MAX_VALUE)? -1: dis[a][b] );
  }
    }
