...
public class entry_9230606 {
    ...
    public static void main(String args[]) throws IOException {
        ...
        for (int i = 0; i < q; i++) {
            int qa = reader.nextInt() - 1;
            int qb = reader.nextInt() - 1;
            long atb = distances[qa][qb];

            writer.println(atb == 0x3f3f3f3f3f3f3f3fL ? -1 : atb);
        }
    ...
    */
    ...
    public long[][] floydWarshall() {
        ...
        for (int i = 0; i < vertexCount; i++) {
            ...
            distances[i][i] = 0;
            for (Edge edge : adj.get(i)) {
                distances[i][edge.destination] = Math.min(distances[i][edge.destination], edge.weight);
            }
        }
