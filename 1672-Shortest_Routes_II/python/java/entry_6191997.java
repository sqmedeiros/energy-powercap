...
public class entry_6191997 {
    static In in = new FastIn();
    static Out outt = new Out(false);
    ...
    void solve() {
     int n = inp() , k = inp() , q = inp() , temp = q;

     long[][] arr = new long[n][n];

     for(long[] a : arr) Arrays.fill(a , inf);
  ...
  for(int i =0  ; i < n; i++) arr[i][i] = 0;     
     for(int via = 0 ; via < n ; via++){
      for(int i = 0 ; i < n ; i++){
       for(int j = 0 ; j < n ; j++){
        if (arr[i][via] < inf && arr[via][j] < inf)
