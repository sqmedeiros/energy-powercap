...
public class entry_2983741 {
    ...
    static final long INF = Long.MAX_VALUE / 1000;
    ...
    static long[][] floyd(long[][] graph, int n){
        long[][] A = new long[n][n];
        for (int i = 0; i < n; i++){
            for (int j = 0; j < n; j++){
                A[i][j] = graph[i][j];
        ...
        for (int k = 0; k < n; k++){
            for (int u = 0; u < n; u++){
                for (int v = 0; v < n; v++){
                    if(A[u][k] != Long.MAX_VALUE && A[k][v] != Long.MAX_VALUE && A[u][v] > A[u][k] + A[k][v]){
                        A[u][v] = A[u][k] + A[k][v];
                    }
                }
            }
