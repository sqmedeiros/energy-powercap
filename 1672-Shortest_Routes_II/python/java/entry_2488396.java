...
public class entry_2488396 {
 static long MAX = Long.MAX_VALUE / 2;

 public static void main(String[] args) {
  solve();
  // solve_accepted_but_not_correct();
 }

 static void solve_accepted_but_not_correct() {
  int n = ni(), m = ni(), q = ni();
 ...
 static void solve() {
  ...
  for (int i = 0; i < m; i++) {
   int u = ni() - 1, v = ni() - 1, w = ni();
   matrix[v][u] = matrix[u][v] = Math.min(matrix[u][v], w);
  }

  for (var i = 0; i < n; i++) {
   for (var j = 0; j < n; j++) {
    for (var k = j + 1; k < n; k++) {
     matrix[j][k] = matrix[k][j] = Math.min(matrix[j][k], matrix[j][i] + matrix[i][k]);
    }
   }
  }
