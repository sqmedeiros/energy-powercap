...
def carregacsv(arquivo, flags, nexec):
    ...
    if ncolunas == 14:
        df = pandas.read_csv(arquivo,names=['codigo','psys','pkg','cores','gpu', 'ram', 'duration_time','user_time','system_time','perf_time','lang','machine','ncores','power_limit'])
    else:
...
def calculamedias(df, ncolunas, nlinhas, nexec, flags, file):
    ...
    while j < nlinhas:
        ...
        pkg = np.array(df.iloc[j:j+nexec,2]).astype(float)
