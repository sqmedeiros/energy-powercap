...
//        int n = sc.nextInt();
        ...
        for(int i=1;i<=n;i++)
        {
            long a = sc.nextLong(),k=1,sum=0;
            while(k<=a){
                long temp = (a/k),l = a/temp,s=k+l,count=l-k+1;
                if(s%2==0) s/=2;
                else count/=2;
                s = ((s%mod)*(count%mod))%mod;
                //out.println(count+" "+l+" "+s);
                sum=((sum%mod)+((s%mod)*(temp%mod))%mod)%mod;
                k=l+1;
            }
            out.println(sum);
        }
