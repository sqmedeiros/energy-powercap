...
public class entry_5186402 {static long mod=(long)1e9+7;
    ...
    static long modexp(long a, long b) {long res = 1;while (b > 0) {if ((b & 1) != 0) res = (res * a) % mod ;a = (a  * a ) % mod;b >>= 1;}return res ;}
    ...
    //---------------------------------------------------------------------------------------------------------------------------------------//
    static long total_sum(long start, long end) {
        return (
                ((((end - start + 1) % mod) * ((start + end) % mod) % mod)*500000004)%mod);
