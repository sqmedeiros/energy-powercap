...
public class entry_10699492 {
    ...
    public static void main (String[] args) throws Exception {
        ...
        int n=io.nextInt(),k=io.nextInt(),res=0;
        int[][] mat=new int[n][2];
        for(int i=0;i<n;i++) mat[i]=io.readArray(2);
        ...
        for(int i=0;i<n;i++){
            ...
            if (lower!=null){
                res++;
                map.put(mat[i][1],map.getOrDefault(mat[i][1],0)+1);
                map.put(lower,map.get(lower)-1);
                if (map.get(lower)==0) map.remove(lower);
            }
        }
        io.println(res);
        //}
    ...
    public static boolean f(long[] a,long x,long k){
        long req=1,curr=0;
        for(long i:a){
            if (i>x) return true;
            if (curr+i>x){
