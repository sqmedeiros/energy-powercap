...
public class entry_759673 { 
    ...
    static class Solver {
        public void solve(FastScanner in, PrintWriter out) {
            ...
            RandomTree<Integer> set = new RandomTree<>(random);
            set.insert(0).count = k;

            int ans = 0;
            for(int i = 0; i<n; i++){
                Tuple movie = input.get(i);
                RandomTree<Integer>.Node person = set.floor(movie.b);
        ...
        private static class RandomTree <T extends Comparable<T>> {
            class Node{
                T key;
                int count, priority;
            ...
            public Node insert(T key){
                ...
                while(cur != null){
                    ...
                    par = cur;
                    if(newNode.key.compareTo(cur.key) < 0) cur = cur.left;
                    else                      cur = cur.right;
                }
                if(par == null)                 root = newNode;
                else if(newNode.key.compareTo(par.key) < 0)  par.left = newNode;
                else                            par.right = newNode;
                return newNode;
            }

            public Node erase(T key){
                Node cur = root, par = null;
                while(cur != null){
                    if(cur.key == key){
                        if(par == null)        root = merge(cur.left, cur.right); 
                        else if(key.compareTo(par.key) < 0) par.left = merge(cur.left, cur.right);
            ...
            private void split(Node subtree, Node separator) {
                ...
                if(separator.key.compareTo(subtree.key) < 0){
