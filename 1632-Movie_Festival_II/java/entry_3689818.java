...
//   mov[i][0] = Integer.parseInt(st.nextToken());
//   mov[i][1] = Integer.parseInt(st.nextToken());
//  }
//  
//  Arrays.sort(mov, (int[] a, int[]b) -> Integer.compare(a[1], b[1]));
//  
...
//  
//  while(i < N) {
//   if(members.floorKey(mov[i][0]) != null) {
//    int key = members.floorKey(mov[i][0]);
//    members.put(key, members.get(key) - 1);
//    if(members.get(key) == 0) members.remove(key);
//    members.putIfAbsent(mov[i][1], 0);
//    members.put(mov[i][1], members.get(mov[i][1]) + 1);
//    movWatched++;
//   }
//   i++;
//  }
//  
//  System.out.println(movWatched);
// }
//}
