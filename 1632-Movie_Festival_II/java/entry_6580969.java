...
public class entry_6580969 {
    void solve(Reader s) throws Exception {
        ...
        tickets.put(0, k);
        for (int i = 0; i < n; i++) {
            Integer floor = tickets.floorKey(movies.get(i).getA());
            //            out.println(floor);
            if(floor != null) {
                tickets.replace(floor, tickets.get(floor) - 1);
                if(tickets.get(floor) == 0) tickets.remove(floor);
                if(tickets.containsKey(movies.get(i).getB())) {
                    tickets.replace(movies.get(i).getB(), tickets.get(movies.get(i).getB()) + 1);
                } else {
                    tickets.put(movies.get(i).getB(), 1);
                }
                count++;
            }
        }
    ...
    public static void main(String[] args) {
        try {
            Reader s = new Reader();
            entry_6580969 obj = new entry_6580969();
            obj.solve(s);
        } catch (Throwable e) {
            e.printStackTrace();
            exit(1);
