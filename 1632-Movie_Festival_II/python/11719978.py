...
class FenwickTree:
    ...
    def find_largest_up_to_bound(self, bound: int) -> int | None:
        ...
        # First, we find sum(arr[:bound+1]). We will use this to compare with
        # sum(arr[:i]) for different values of i to find the required index. If
        # and only if sum(arr[:bound+1]) == 0, there are no indices satisfying
        # the required constraints.
        remaining = self.sum_indices_up_to(bound)
        if remaining == 0:
            return None
    
        # We use the binary indices of the tree to recover the largest index.
        # Reference for this technique: https://codeforces.com/blog/entry/61364.
        mask = bit_ceil(len(self.tree) - 1)
        index = 0
        while mask > 0:
            new_index = index + mask - 1
            if new_index < len(self.tree) and self.tree[new_index] < remaining:
                remaining -= self.tree[new_index]
                index += mask
            mask >>= 1
        return index
