...
#
#
# See part 1 first [G2 Q6 - Movie Festival] (https://cses.fi/problemset/task/1629)
# This is a task scheduling problem, goal is to maximize tasks completed, but we can work on K of them simultaneously
# TC: O(nlogn)
...
# sort only indices to avoid storing tuples, this is faster
left = []
right = []
for _ in range(n):
    ...
    left.append(l)
    right.append(r)
indices = sorted(range(n), key=lambda i: right[i])
 
# arr = [tuple(map(int, input().split())) for _ in range(n)]  # [l,r) intervals
#
# arr.sort(key=lambda x:x[1])
 
exist = SortedList([-1] * k)  # end times of previous workers
 
total = 0
for i in indices:
 
    idx = exist.bisect_right(left[i])-1
    if idx >= 0:
        exist.pop(idx)
        exist.add(right[i])
        total += 1
