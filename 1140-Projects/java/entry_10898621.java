...
public class entry_10898621 {
    ...
    static void solve() throws IOException {
        ...
        long max = 0;
        for(long [] e : a){
            Long key = map.lowerKey(e[0]);
            if(key == null){
                max = Math.max(max,e[2]);
            }else{
                max = Math.max(max,map.get(key)+e[2]);
            }
            max = Math.max(max,map.getOrDefault(e[1],max));
            map.put(e[1],max);
        }
        pl(map.values().stream().max(Long::compare).get());
    }
