...
public class entry_2568525 {
    ...
    static class Solver {
        ...
        void solve() {
            int n = io.nextInt();
            PriorityQueue<IDval> upcomingSegments = new PriorityQueue<>(n);
            ...
            for (int i=0; i<n; i++) {
                ...
                ends[i] = io.nextInt();
                prices[i] = io.nextInt();
                upcomingSegments.add(new IDval(i, starts[i]));
            }

            PriorityQueue<IDval> endingSegments = new PriorityQueue<>();
            long maxPriceWithoutLockIn = 0;
            while (!endingSegments.isEmpty() || !upcomingSegments.isEmpty()) {
                int upcomingSegmentTime = (upcomingSegments.isEmpty() ? Integer.MAX_VALUE : upcomingSegments.peek().time);
                if (!endingSegments.isEmpty() && endingSegments.peek().time <= upcomingSegmentTime) {
                    IDval processedEvent = endingSegments.poll();
                    maxPriceWithoutLockIn = Math.max(maxPriceWithoutLockIn, processedEvent.price);
                } else {
                    IDval processedEvent = upcomingSegments.poll();
                    int i = (int) processedEvent.price; // not price
                    IDval endingEvent = new IDval(maxPriceWithoutLockIn + prices[i], ends[i]+1);
                    endingSegments.add(endingEvent);
                }
            }
            io.println(maxPriceWithoutLockIn);
        }
        ...
        class IDval implements Comparable<IDval> {
            ...
            public IDval(long price, int time) {
                this.time = time;
                this.price = price;
            }
