...
public class entry_13578767 {
    ...
    public static void main(String[] args) {
        ...
        HashMap<Integer, Queue<int[]>> right = new HashMap<>();
        for (int i = 0; i < n; i++) {
            for (int j = i + 1; j < n; j++) {
                right.computeIfAbsent(arr[i] + arr[j], k -> new LinkedList<>()).add(new int[]{i, j});
            }
        }

        for (int i = 0; i < n; i++) {
            for (int j = i + 1; j < n; j++) {
                right.get(arr[i] + arr[j]).poll();
            }
            for (int j = 0; j < i; j++) {
                int goal = target - arr[i] - arr[j];
                Queue<int[]> queue = right.get(goal);
                if (queue != null && !queue.isEmpty()) {
                    println((j + 1) + " " + (i + 1) + " " + (queue.peek()[0] + 1) + " " + (queue.peek()[1] + 1));
                    flush();
                    return;
                }
            }
