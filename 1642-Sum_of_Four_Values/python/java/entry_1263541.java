...
import java.text.*;
public class entry_1263541 implements Runnable {
    ...
    public static class Pair implements Comparable<Pair> {
        ...
        public int compareTo(Pair p) {
            return (int)this.val - (int)p.val;
        }
    }


    public static void solve() {
        int n = f.ni(); int x = f.ni();

        Pair arr[] = new Pair[n];

        for (int i = 0; i < n; ++i) {
            arr[i] = new Pair(f.nl() , i);
        }
        ...
        for (int i = 0; i < n; ++i) {
            for (int j = i + 1; j < n; ++j) {
                int lo = j + 1 , hi = n - 1;
                while (lo < hi) {
                    if (arr[i].val + arr[j].val + arr[lo].val + arr[hi].val == x) {
                        p((arr[i].idx + 1) + " ");
                        p((arr[j].idx + 1) + " ");
                        p((arr[lo].idx + 1) + " ");
                        p((arr[hi].idx + 1) + " ");
                    ...
                    else if (arr[i].val + arr[j].val + arr[lo].val + arr[hi].val < x) ++lo;
                    else --hi; 
                }
            }
        }
    ...
    public static void main(String[] args)throws IOException {
        new Thread(null , new entry_1263541() , "F*uckit" , 1 << 28).start();  
    }
