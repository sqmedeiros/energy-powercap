...
public class entry_3576766 {

    static class Pair {
        int x, y;
        Pair(int x, int y) { this.x = x; this.y = y; }
    }
    public static void sum_of_four_values(int n, int t, int[] a, PrintWriter out) {
        HashMap<Integer, Pair> map = new HashMap<>();
        for (int i = 0; i < n; ++i) {
            for (int j = i + 1; j < n; ++j) {
                if (map.containsKey(t - a[i] - a[j])) {
                    Pair p = map.get(t - a[i] - a[j]);
                    out.printf("%d %d %d %d\n", i + 1, j + 1, p.x + 1, p.y + 1);
                    return;
                }
            }
    ...
    static class FastScanner {
        ...
        public FastScanner(String s) {
            try { in = new BufferedInputStream(new FileInputStream(new File(s)), BS); }
            catch (Exception e) { in = new BufferedInputStream(System.in, BS); }
        }
