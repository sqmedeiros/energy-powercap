...
class UnionFind {
    ...
    private int find(int x) {
        ...
        compressPath(x, parent);
        return parent;
    }

    private void compressPath(int x, int root) {
        while (parent[x] != root) {
            int parent = this.parent[x];
            this.parent[x] = root;
            x = parent;
    ...
    public void printRes() throws IOException {
        ...
        writer.write((roots.size() - 1 + "\n").getBytes());
        for (int i = 0; i < roots.size() - 1; i++) {
            int from = roots.get(i) + 1;
            int to = roots.get(i + 1) + 1;
...
public class entry_1511852 {
    ...
    private static void solve(List<List<Integer>> graph) throws IOException {
    ...
    private static void printRes(int[] parent) throws IOException {
        OutputStream writer = new BufferedOutputStream(System.out);
        ...
        while (!path.isEmpty()) {
            writer.write((path.pop() + " ").getBytes());
        }
        writer.write("\n".getBytes());
        writer.flush();
    }
