...
//        int tests = scanInt();
        ...
        for (int test = 0; test < tests; test++) {
            ...
            for(int i = 0; i < q; i++){
                ...
                out.println(st.query(0, n-1).max);
            }
...
class pair {
    ...
    long right;
    long sum;
    pair(long max, long left, long right, long sum){
        this.max = max;
        this.left = left;
        this.right = right;
        this.sum = sum;
...
class SegmentTree{
    ...
    private pair operation(pair a, pair b){
        long m = Math.max(a.max, Math.max(b.max, a.right + b.left));
        long l = Math.max(a.left, a.sum + b.left);
        long r = Math.max(a.right + b.sum, b.right);
        long s = a.sum + b.sum;
        return new pair(m, l, r, s);
    }
    ...
    private void build(int []arr, int ind, int low, int high){
        if(low == high){
            long val = Math.max(0L, arr[low - 1]);
            tree[ind] = new pair(val, val, val, arr[low - 1]);
            return;
        }
        int mid = low + (high - low)/2;
        build(arr, 2*ind, low, mid);
    ...
    private pair querySearch(int ind, int low, int high, int l, int r){
        if(l > high || r < low){
            return new pair((int)1e9 + 7, (int)1e9 + 7, (int)1e9 + 7, (int)1e9 + 7);
        }
        if(low >= l && high <= r){
            return tree[ind];
        }
        int mid = low + (high - low)/2;
        pair left = querySearch(2*ind, low, mid, l, r);
        pair right = querySearch(2*ind + 1, mid+1, high, l, r);
        return operation(left, right);
    }
 
    public pair query(int l, int r){ // asked for 0-based indexing
        return querySearch(1, 1, n, l + 1, r + 1); // converted to 1-based indexing
    }
    ...
    private void pointUpdate(int ind, int low, int high, int pos, int val){ // works in 1-based indexing
        if(low == high){
            long num = Math.max(0L, val);
            tree[ind] = new pair(num, num, num, (long) val);
        }
        else {
            int mid = low + (high - low) / 2;
            if (pos <= mid && pos >= low) {
