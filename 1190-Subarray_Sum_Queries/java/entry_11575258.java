...
public class entry_11575258 {
    ...
    static class Node{
        ...
        Node(){
            ...
            pref = Integer.MIN_VALUE;
            suff = Integer.MIN_VALUE;
        }
        Node(int v, int sum, int p, int s){
            val = v;
            this.sum = sum;
            pref = p;
            suff = s;
        ...
        void merge(Node left, Node right){
            val = Math.max(Math.max(left.val , right.val), left.suff + right.pref);
        ...
        void updateValue(int start, int end, Update newUpdate){
            val = Math.max(0, newUpdate.val);
            sum = newUpdate.val;
            pref = Math.max(0, newUpdate.val);
            suff = Math.max(0, newUpdate.val);
        ...
        // while(t --> 0){
            ...
            for(int i = 0; i<q; i++){
                ...
                // if(query == 1){
                    ...
                    // System.out.println(sgt.query(0, n - 1, a, a, 1).val);
                    ans.append(sgt.query(0, n - 1, 0, n - 1, 1).val+"\n");
                // }
                // else{
                    // int a = sc.nextInt() - 1;
                    // // int b = sc.nextInt() - 1;
                    // long currDown = down.query(0, n - 1, 0, a, 1).val + a;
                    // long currUp = up.query(0, n - 1, a, n - 1, 1).val - (a);
                    // // System.out.println(currDown+ " "+currUp);
                    // long currAns = Math.min(currUp, currDown);
                    // ans.append(currAns +"\n");
                // }
            }
