...
public class entry_4163337 {
    ...
    static class SegTree{
        ...
        public SegTree(int[] arr, int n){
            segTree = new long[4*n][4]; // {full, prefix, suffix, max}
            build(0,n-1,0,arr);
        }
        private void merge(int sI,int lC,int rC){
            segTree[sI][1] = Math.max(segTree[lC][1],segTree[lC][0] + segTree[rC][1]);
            segTree[sI][2] = Math.max(segTree[rC][2],segTree[rC][0] + segTree[lC][2]);
            segTree[sI][0] = segTree[lC][0] + segTree[rC][0];
            segTree[sI][3] = Math.max(Math.max(segTree[lC][3],segTree[rC][3]),segTree[lC][2]  + segTree[rC][1]);
        }
        private void build(int sL, int sR, int sI,int [] arr){
            if(sL == sR){
                segTree[sI] = new long[]{arr[sL],Math.max(0,arr[sL]),Math.max(0,arr[sL]),Math.max(0,arr[sL])};
                return;
            }
            int lC = (sI<<1) + 1;
            int rC = (sI << 1) + 2;
            int mid = ((sL + sR)>>1);
            build(sL,mid,lC,arr);
            build(mid+1,sR,rC,arr);
            merge(sI,lC,rC);
 
        }
        public void update(int sL, int sR, int sI, int i,int val){
            if(i<sL || i>sR)return;
            if(sL == sR){
                segTree[sI] = new long[]{val,Math.max(0,val),Math.max(0,val),Math.max(0,val)};
                return;
            }
            int lC = (sI<<1) + 1;
            int rC = (sI << 1) + 2;
            int mid = ((sL + sR)>>1);
            update(sL,mid,lC,i,val);
            update(mid+1,sR,rC,i,val);
            merge(sI,lC,rC);
        }
    ...
    public static void solve(int tCase) throws IOException {
        ...
        for(int i=0;i<m;i++){
            int y = sc.nextInt();
            int z = sc.nextInt();
            st.update(0,n-1,0,--y,z);
            out.println(st.segTree[0][3]);
        }
    }
...
//        testCase = sc.nextInt();
    ...
    public static int inf_int = (int) 2e9 + 1000;
