...
public class entry_9685535 {
    ...
    static void run(InputStream in, PrintStream out) throws IOException {
        ...
        var n = scanner.nextUlong();
        var k = scanner.nextUint();
        var as = scanner.nextUlongs(k);
        out.print(solve(n, as));

        out.flush();
    }

    static long solve(long n, long[] as) {
        var res = 0L;
        for (int mask = 1; mask < (1 << as.length); mask++) {
            var bitCount = Long.bitCount(mask);

            var prod = 1L;
            for (int i = 0; i < as.length; i++) {
                if ((mask & (1 << i)) != 0) {
                    prod = multiplyCapped(prod, as[i]);
                }
            }
    ...
    private static long multiplyCapped(long x, long y) {
        long r = x * y;
        if (((x | y) >>> 31 != 0)) {
            if (((y != 0) && (r / y != x))) {
    ...
    @SuppressWarnings({"DuplicatedCode", "unused"})
    static final class BinaryGrid {
        private final BitSet[] bits;
        private final int w;

        BinaryGrid(BitSet[] bits, int w) {
    ...
    @SuppressWarnings({"DuplicatedCode", "unused"})
    static final class BinaryString {
        private final BitSet bits;
        private final int length;

        public BinaryString(BitSet bits, int length) {
            if (length < 0) throw new IndexOutOfBoundsException("" + length);
            this.bits = bits;
            this.length = length;
        }
