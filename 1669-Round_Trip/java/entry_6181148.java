...
public class entry_6181148 {
 ...
 static boolean[] vis;
    ...
    static boolean slag = false;

    void solve() {
     n = inp();
     k = inp();
     adj = in.nextEdges(n , k , false);
     ...
     for(int i = 0 ; i < n ; i++){
      if(!vis[i]){
       ...
       if(cc){
        ...
        int x = 2;
        for(int j = start ; j != end ; j = parent[j]){
         list.add(j);
         x++;
        }
        list.add(end);
        ...
        sb.append(x + "\n");
        for(int ii : list) sb.append((ii+1) + " ");
        out.println(sb);
        return;
       }
      }
