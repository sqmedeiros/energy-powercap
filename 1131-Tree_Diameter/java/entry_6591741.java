...
public class entry_6591741 {
  ...
  int ans;
    ...
    void solve() {
     int n = in.nextInt();
     List<List<Integer>> adj = in.nextEdges(n , n-1 , false);
     ...
     // first -> node
     // second -> distance from the lastNode node
     Deque<IntPair> q2 = new ArrayDeque<>();
     BitSet bs2 = new BitSet();

     bs2.set(lastNode);
     q2.offer(new IntPair(lastNode , 0));

     int max = 0;

     while(!q2.isEmpty()){
      IntPair e = q2.pollFirst();

      max = Math.max(max , e.second);

      for(int it : adj.get(e.first)){
       if(!bs2.get(it)){
        bs2.set(it);
        q2.offer(new IntPair(it , e.second + 1));
       }
      }
     }
