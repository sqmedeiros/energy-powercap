...
# negative cycle present
    ...
    while True:
        ...
        if start == y and len(path) > 1:
            break
        start = relaxant[start]

    # reversing in this last to get the correct path
    print(*path[::-1], sep=" ")

    # if a is relaxant of b that means there is path from a to b when we are following relaxant order b will come before a but in actuality a should come before b