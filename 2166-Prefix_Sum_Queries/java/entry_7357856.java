...
class SegmentTree{
    ...
    private pair operation(pair a, pair b){
        return new pair(Math.max(a.pre, a.sum + b.pre), a.sum + b.sum);
    }
    ...
    private void build(int []arr, int ind, int low, int high){
        if(low == high){
            tree[ind] = new pair(Math.max(0L, arr[low - 1]), arr[low - 1]);
            return;
        }
        int mid = low + (high - low)/2;
        build(arr, 2*ind, low, mid);
    ...
    private pair querySearch(int ind, int low, int high, int l, int r){
        ...
        int mid = low + (high - low)/2;
        pair left = querySearch(2*ind, low, mid, l, Math.min(r, mid));
        pair right = querySearch(2*ind + 1, mid+1, high, Math.max(l, mid+1), r);
        return operation(left, right);
    }
    ...
    private void pointUpdate(int ind, int low, int high, int pos, int val){
        if(low == high){
            tree[ind].sum = (long)val;
            tree[ind].pre = Math.max(0L, (long) val);
        }
        else {
            int mid = low + (high - low) / 2;
            if (pos <= mid && pos >= low) {
