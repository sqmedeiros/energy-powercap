...
public class entry_2001070 {
 ...
 public static void main(String args[]) throws Exception {
  ...
  ArrayList<ArrayList<Integer>> graph = new ArrayList<>(pupils);
  for(int i = 0; i < pupils; i++)graph.add(new ArrayList<>());
 ...
 private static String getTeams(ArrayList<ArrayList<Integer>> graph){
 ...
 private static boolean dfs(int i, ArrayList<ArrayList<Integer>> graph, int parent){
  if(!visited[i]){
   visited[i] = true;
   ...
   if(team[parent] == team[i]) return false;
  }
  return true;
 }
}