    ...
    static class QuickSort <T extends Comparable<T>> {
        ...
        public void sort(){
            ...
            for(int i = 0; i<pos.length; i++){
                if(pos[i] == -1) continue;
                int cur = i;
                // new Debugger().d("starting at "+i).d();
                do{
                    int cur2 = pos[cur];
                    pos[cur] = -1;
                    if(cur2 == i) break;
                    {T swapTemp = arr[cur]; arr[cur] = arr[cur2]; arr[cur2] = swapTemp;}
                    // new Debugger().d(arr).d();
                    cur = cur2;
                }while(true);
            }
        }
