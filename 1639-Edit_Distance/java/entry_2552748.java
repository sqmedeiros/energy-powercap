        ...
        void solve() {
            ...
            for (int i=0; i<=n; i++) {
                for (int j=0; j<=m; j++) {
                    ...
                    int replaceMatchCost = (i<n && j<m && a.charAt(i) == b.charAt(j) ? 0 : 1);
                    dp[i+1][j+1] = Math.min(dp[i+1][j+1], dp[i][j] + replaceMatchCost); // Match characters at i and j (replace the ith character from a if necessary)
                    dp[i+1][j] = Math.min(dp[i+1][j], dp[i][j] + 1); // Remove ith character from a
                    dp[i][j+1] = Math.min(dp[i][j+1], dp[i][j] + 1); // Add character to beginning of a (character matching jth character from b)
                }
            }
            //draw(dp);
            io.println(dp[n][m]);
