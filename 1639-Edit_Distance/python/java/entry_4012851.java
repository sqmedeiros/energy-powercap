...
public class entry_4012851 {
  ...
  static class sort implements Comparator<pair>{
   ...
   public int compare(pair o1, pair o2) {
    ...
    if(o1.a == o2.a) {
     if(o1.b > o1.b) return 1;
     else if(o1.b == o1.b) return 0;
     else return -1;
    }
    else return -1;
   }
  ...
  static long recurse1(String s1, String s2, int i, int j, Long [][] dp) {
   if(i == s1.length() && j == s2.length()) return 0;
   if(i == s1.length()) return s2.length() - j;
   if(j == s2.length()) return s1.length() - i;
   if(dp[i][j] != null) return dp[i][j];
   if(s1.charAt(i) == s2.charAt(j)) return recurse1(s1, s2, i + 1, j + 1, dp);
   long insert = recurse1(s1, s2, i, j + 1, dp) + 1;
   long remove = recurse1(s1, s2, i + 1, j, dp) + 1;
   long replace = recurse1(s1, s2, i + 1, j + 1, dp) + 1;
