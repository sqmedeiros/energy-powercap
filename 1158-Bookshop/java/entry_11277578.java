        ...
        try
        {
            ...
            //     testCases--;
                ...
                for (int ind=n-1;ind>=0;ind--)
                {
                    ...
                    {
                        int not_buy = dp[ind + 1][rem];
                        int buy = 0;
                        if (h[ind] <= rem)
                        {
                            buy = s[ind] + dp[ind + 1][rem - h[ind]];
                        }
                        dp[ind][rem] = Math.max(not_buy, buy);
                    }
                }
                int maxNumOfPages = dp[0][x];
                out.println (maxNumOfPages);
            // }
        }catch(Exception e){
            return;
        }
