...
public class entry_15536424 { 
    /*
        Cool Tree DP problem. Required having two different kinds of DP definitions.
         dp(i) = maximum 2 distances within the same sub-tree that go through different children
        dp2(i) = maximum distance that goes through the parent of i
         Initial attempts were recursive and more straight forward, however, Java timed out.
         To get it to run in time I needed to unwind the recursion and implement it iteratively.
                The key to doing this was to run an initial linear topological sort
            > from this I could now build the DP in the right order iteratively
            > I also needed to build the parent[] array in order to use it in the dp relation
                dp(i) = max 2 of {1+dp(child_i)}
        dp2(i) = if you have a parent
            1 + dp(parent)  --> note if you're the best option for your parent then you need to pick option 2
            or
            1 + dp2(parent) whichever is best
                final answer for each node i = max(dp[i], dp2[i])
     */
