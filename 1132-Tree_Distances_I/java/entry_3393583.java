...
public class entry_3393583 {
    ...
    static class TaskC
    {
        ...
        static int mx_dist[]=new int[200001];
        ...
        private static Pair bfs(int src)
        {
            ...
            while(!q.isEmpty())
            {
                curr=q.poll();
                vis[curr.x]=true;
                mx_dist[curr.x]=Math.max(mx_dist[curr.x],curr.y);
                for(Integer child:adj[curr.x])
                {
                    if(!vis[child])
                    {
                        q.add(new Pair(child,curr.y+1));
                    }
                }
            }
            return curr;
