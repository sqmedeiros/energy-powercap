...
def get_configuration ():
  """ Returns the configuration of the current experiment. """
  run_config = {}
...
def make_head_csv (csv_file, events, run_config):
  if os.path.isfile(csv_file):
    return

  print(f"Config {config}")
  fields = [ "file" ] + events + list(run_config.keys())
...
run_config = get_configuration()
make_head_csv(csv_file, list_events + list_times + ["perf_time"], run_config)

run_config_values = [str(value) for value in run_config.values()]
print("Values: ", run_config_values)
...
for i in range(NRUNS):
  ...
  make_new_csv_entry(csv_file, entry_data + run_config_values)   
