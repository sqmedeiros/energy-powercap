...
public class entry_10082871 {
    ...
    static long[] dj(int source, ArrayList<int[]>[] adj){
        ...
        while (!pq.isEmpty()){
            ...
            if(!visit[node]){
                ...
                visit[node] = true;
                for(int[] v : adj[node]){
                    dist[v[0]] = Math.min(dist[v[0]] , dist[node] + v[1]);
                    if(!visit[v[0]]) pq.add(new long[]{dist[v[0]], v[0]});
                }
            }
        }
    ...
    public static void main(String[] args)throws IOException{
        ...
        for(int i=1;i<=n;i++){
            for(int[] v: adj[i]){
                if(d1[i] + dn[v[0]] + (v[1]/2) < min){
                    min = d1[i] + dn[v[0]] + (v[1]/2);
                }
            }
        }
        out.println(min);
