...
class entry_1411015 {
  static class pair implements Comparable<pair>{
    ...
    public int compareTo(pair p){
      if(weight==p.weight) return Integer.compare(end, p.end);
      return Long.compare(weight, p.weight);
    }
  }

  static long[] dj(ArrayList<ArrayList<pair>> graph, int start){
    ...
    while(!pq.isEmpty()){
      pair p= pq.poll();
      if(dist[p.end]!=p.weight) continue;
      for(pair temp: graph.get(p.end)){
        if(dist[temp.end]> dist[p.end]+temp.weight){
          dist[temp.end]=dist[p.end]+temp.weight;
          pq.add(new pair(temp.end, dist[temp.end]));
        }
      }
    }
  ...
  static class ProblemSolver{
    public void solveTheProblem(InputReader in,OutputWriter out, NumberTheory nt, Debug dbg){
      ...
      while(test-- >0){
        ...
        ArrayList<ArrayList<pair>> graphRev =new ArrayList<>();
        for(int i=0;i<=n;i++){
          graph.add(new ArrayList<>());
          graphRev.add(new ArrayList<>());
        }
        // long dis[][]= new long[graph.size()][graph.size()];
        // System.out.println(graph.size());
        // for(long[] i: dis) Arrays.fill(i, Integer.MAX_VALUE);
        for(int i=0;i<m;i++){
          int a=in.nextInt();
          int b=in.nextInt();
          long c=in.nextLong();
          ...
          graphRev.get(b).add(new pair(a, c));
        }
        // for(long []l: dis) System.out.println(Arrays.toString(l));
        // System.out.println(graph);
        long dist1[]= dj(graph, 1);
        long dist2[]= dj(graphRev, n);
        // long ans=Long.MAX_VALUE;
        BigInteger ans= null;
        boolean first=true;
        for(int ii=1;ii<=n;ii++){
          for(pair p: graph.get(ii)){
            int i=p.end;
            // System.out.println("p: "+p);
            // System.out.printf("dist1[i]: %d\tdist2[i]: %d\t p.weight: %d\tp.end:: %d\tval: %d\t i: %d\t\n",dist1[i], dist2[i], p.weight, p.end, (dist1[i]+dist2[i]+p.weight/2), i);
            // System.out.println("first: "+first);
            if(first){
              // System.out.println("First");
              ans=BigInteger.valueOf(dist1[ii]).add(BigInteger.valueOf(dist2[i])).add(BigInteger.valueOf(p.weight/2));
              first=false;
              continue;
            }
            if(ans.compareTo(BigInteger.valueOf((dist1[ii]+dist2[i]+p.weight/2)))>0){
              // System.out.println("BIG: "+(BigInteger.valueOf((dist1[ii]+dist2[i]+p.weight/2))));
              // System.out.printf("dist1[i]: %d\tdist2[i]: %d\t p.weight: %d\t ans: %d\tval: %d\t\n",dist1[i], dist2[i], p.weight, ans, (dist1[i]+dist2[i]+p.weight/2), p.end, i);
              ans=BigInteger.valueOf(dist1[ii]).add(BigInteger.valueOf(dist2[i])).add(BigInteger.valueOf(p.weight/2));
            // if(ans>(dist1[ii]+dist2[i]+p.weight/2)){
            }
          }
        }
