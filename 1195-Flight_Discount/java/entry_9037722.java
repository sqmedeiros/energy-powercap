 ...
 * DON'T GET STUCK ON ONE APPROACH
    */
    ...
    public static void solve()throws Exception
    {
        ...
        for(int i=0;i<n;i++)dis1[i]=(long)(1e15);
        dis1[0]=0;
        for(int i=0;i<m;i++)
        {
            int a=in.nextInt();
            int b=in.nextInt();a--;b--;
            int c=in.nextInt();
            al.add(new Pair(a,b,c));
            g1.get(a).add(new Edge(b,c));
            g2.get(b).add(new Edge(a,c));
        }
        PriorityQueue<Edge> pq = new PriorityQueue<>(new compareEdge());
        pq.add(new Edge(0,0));
        while(pq.size()>0)
        {
            int node=pq.peek().a;
            ...
            pq.remove();
            if(weight>dis1[node])continue;
            for(Edge child:g1.get(node))
            {
                long w1=child.weight;
                int adj=child.a;
                if(weight+w1<dis1[adj])
                {
                    dis1[adj]=weight+w1;
                    pq.add(new Edge(adj,weight+w1));
                }
            }
        }
        long dis2[]=new long[n];
        for(int i=0;i<n;i++)dis2[i]=(long)(1e14);
        dis2[n-1]=0;
        pq = new PriorityQueue<>(new compareEdge());
        ...
        while(pq.size()>0)
        {
            ...
            long weight=pq.peek().weight;
            pq.remove();
            if(weight>dis2[node])continue;
            for(Edge child:g2.get(node))
            {
                long w1=child.weight;
                int adj=child.a;
                if(weight+w1<dis2[adj])
                {
                    dis2[adj]=weight+w1;
                    pq.add(new Edge(adj,weight+w1));
                }
        ...
        {
            Pair p=al.get(i);
            ans=min(ans,dis1[p.a]+(p.c/2)+dis2[p.b]);
        }
        out.println(ans);
    }

    public static void printString(char ch[])throws Exception
    {
        for(char x:ch)out.print(x);
        endl();
    }
