...
public class entry_13775903 { 
    ...
    public static void main(String[] args) throws Exception {
        ...
        var edges = new int[m][];
        for (int i=0; i<m; i++) {
            edges[i] = new int[]{io.nextInt()-1, io.nextInt()-1, io.nextInt()};
        }

        var g = buildGraph(n, edges, false);

        var pq = new PriorityQueue<State>((a,b) -> Long.compare(a.dist, b.dist));
        ...
        while (!pq.isEmpty()) {
            ...
            seen[c.i][c.used] = true;
            if (c.i == n-1) {
                io.out.println(c.dist);
                return;
            }
            for (var next : g[c.i]) {
                if (!seen[next.to][c.used]) {
                    pq.offer(new State(next.to, c.dist + next.weight, c.used));
                }
                if (c.used == 0 && !seen[next.to][1]) {
                    pq.offer(new State(next.to, c.dist + next.weight/2, 1));
                }
            }
        }
    }
    ...
    static class State {
        ...
        public State(int i, long dist, int used) {
            this.i = i;
            this.dist = dist;
            this.used = used;
        }
    ...
    static class FastIO {
        ...
        public FastIO(InputStream inStream, OutputStream outStream) { // typically System.in, System.out
            in = inStream;
            out = new PrintWriter(new BufferedOutputStream(outStream, BUF_SIZE),true); 
            // auto‑flush on println (could be disabled if you want to optimize further -- REMEMBER TO FLUSH YOURSELF)
        }
    ...
    public static Edge[][] buildGraph(int n, int[][] edges, boolean bidirectional) {
        var g = new Edge[n][];
        ...
        for (int[] e : edges) {
            ...
            int v = e[1];
            int w = e[2];
            g[u][--outDegree[u]] = new Edge(v, w);
            if (bidirectional) {
                g[v][--outDegree[v]] = new Edge(u, w);
            }
        }
        return g;
    }
