"""Solution code for "CSES 1091. Concert Tickets".
 - Problem link: https://cses.fi/problemset/task/1091
- Solution link: http://www.teferi.net/ps/problems/cses/1091
 Tags: [ordered set]
"""
...
def main():
    _n, _m = [int(x) for x in input().split()]
    h = [int(x) for x in input().split()]
    t = [int(x) for x in input().split()]

    prices = SortedList(h)
    for t_i in t:
        ind = prices.upper_bound(t_i) - 1
        if ind < 0:
            print('-1')
        else:
            print(prices[ind])
