...
class entry_3833023 { 
  public static void main(String[] args) throws FileNotFoundException {
    ...
    for(int i = 0; i < n; i++) {
      Integer temp = reader.nextInt();
      h.put(temp, (h.get(temp)==null) ? 1 : h.get(temp) + 1);
    }
    ...
    for(int i = 0; i < m; i++) {
      ...
      if (temp == null) {
        ...
        res.append(temp.getKey()).append("\n");
        if (temp.getValue() == 1) {
          h.remove(temp.getKey());
        } else {
          h.put(temp.getKey(), temp.getValue() - 1);
        }
      }
    }
  ...
  public static int binarySearch(int[] arr, int l, int r, int x) {
    if (r >= l) {
      ...
      if (l == r) {
        ...
        else if(mid > 0) return arr[mid - 1];
        else return -1; 
      }
      if (arr[mid] > x)
