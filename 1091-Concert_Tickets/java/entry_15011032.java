...
public static void main(String[]args){entry_15011032 sk_18=new entry_15011032();sk_18.solve();}
public void solve(){
    int t=1;
    while(t-- >0){
      ans();
...
public void ans(){
       ...
       for(int i=0; i< m; i++){
           Integer floor= mp.floorKey(b[i]);
           if(floor == null){
              sb.append( -1 ).append('\n');
           }else{
              sb.append(floor).append('\n');
               int occ= mp.get(floor);
               if(occ == 1){
                  mp.remove(floor);
               }else{
                  mp.put(floor, occ - 1);
